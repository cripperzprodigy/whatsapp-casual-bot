...
### Fixed
- **Fix**: Group chat AI responses now match the user's input language. Previously, `_detect_language()` in `ai_memory_engine.py` short-circuited for groups by returning the static `default_target_language` setting (defaulting to English), completely ignoring the actual message text. The method now performs live `langdetect` detection on the incoming trigger message first, with an LLM-based fallback, and only uses the group default as a last resort. This ensures Indonesian messages get Indonesian replies, etc.
- **Enhancement**: `!contacts list` now utilizes the live WhatsApp network resolution engine (previously exclusive to `!contacts global`) to fetch the most up-to-date user names and privacy settings. The list is processed asynchronously in the background and returns a highly formatted output complete with privacy indicators and alphabetical sorting.
- **Enhancement**: `!contacts export` has been completely refactored to prevent data loss and improve readability. Exports are now written to timestamped files (e.g., `ledger_YYYYMMDD_HHMMSS.csv`) inside `data/exports/groups/`. The SQL query now performs an outer join with `ChatSettings` to enrich the CSV with human-readable "Group Name" columns. The bot now replies with exact statistics on the number of exported contacts and unique groups.
- **Fix**: `!contacts global` has been refactored to query the `GroupContactLedger` database table instead of iterating over the `data/contacts/` file system. The output is now hierarchically grouped by chat, with groups and members sorted alphabetically, rather than presenting a flat list. This preserves context and group association while maintaining background resolution performance.
