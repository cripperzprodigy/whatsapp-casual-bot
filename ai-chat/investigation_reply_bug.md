...
67. **Node.js Webhook contextInfo Forwarding Fix**: Updated `whatsapp-service/src/events.js` to include `msg.contextInfo` in the webhook payload via `...(msg.contextInfo && { contextInfo: msg.contextInfo })`. Previously, the gateway only populated `extendedTextMessage.contextInfo.mentionedJid` but omitted the top-level `contextInfo` containing reply context (`stanzaId`, `participant`, `quotedMessage`). This caused `ReplyContext=False` for all native WhatsApp replies. The fix ensures Python's `extract_context()` receives the full contextInfo object and can detect `is_reply_to_bot` via JID normalization.
