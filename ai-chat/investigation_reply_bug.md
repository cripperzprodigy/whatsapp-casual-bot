...
8. **Serialized ID Resolution (The Final Fix)**: Discovered that `whatsapp-web.js` expects fully serialized IDs (e.g. `false_1234_3EB0...`) instead of short message keys (`3EB0...`). Added a POST endpoint (`/message/resolve-quote-id`) to the Node.js gateway that dynamically searches the local cache to resolve short keys into full serialized IDs before allowing Python to dispatch quoted messages, preventing the final class of `400 Bad Request` failures and providing a graceful plaintext fallback if the message expired from cache.
